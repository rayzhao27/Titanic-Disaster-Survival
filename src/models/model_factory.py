...
class ModelFactory:
    ...
    def evaluate_models(self, models: List[Tuple[str, Any]], X_train, y_train, X_val, y_val) -> Dict[str, Dict[str, float]]:
        results = {}
        ...
        for name, model in models:
            ...
            cv_roc_auc = (
                cross_val_score(model, X_train, y_train, cv=self.config.cv_folds, scoring="roc_auc") if has_proba else None
            )
            ...
            logger.info(
                f"{name} - CV: {cv_accuracy.mean():.4f}, F1: {val_f1:.4f}, ROC-AUC: {roc_auc_str}, PR-AUC: {pr_auc_str}"
            )

        return results

    def create_ensemble(
        self, model_results: Dict[str, Dict[str, float]], X_train, y_train, X_val, y_val
    ) -> Tuple[Any, Dict[str, float]]:

        good_models = [(name, results["model"]) for name, results in model_results.items() if not results["is_overfitting"]]
        ...
        ensemble = VotingClassifier(estimators=good_models, voting=self.config.ensemble_voting)
        ...
        cv_roc_auc = (
            cross_val_score(ensemble, X_train, y_train, cv=self.config.cv_folds, scoring="roc_auc") if has_proba else None
        )
    ...
    def select_best_model(self, model_results: Dict[str, Dict[str, float]]) -> Tuple[str, Any, Dict[str, float]]:
        ...
        if rf_results and abs(rf_results["cv_score"] - best_results["cv_score"]) <= 0.002:
            logger.info(
                f"Using RF model for consistency (CV: {rf_results['cv_score']:.4f}) instead of {best_model_name} (CV: {best_results['cv_score']:.4f})"
            )
